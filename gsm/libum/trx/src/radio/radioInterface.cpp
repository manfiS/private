...
void RadioInterface::pushBuffer(void) {
  ...
  if ((signed)sendCursor > 2*samplesWritten)
...
void RadioInterface::pullBuffer(void)
{
  ...
  unsigned int samplesRead = mRadio->readSamples(shortVector,OUTCHUNK*samplesPerSymbol,&overrun,readTimestamp,&localUnderrun);
...
{
  gsmtrx_log( LOG_INFO, "Starting radio interface..." );
  ...
  mRadio->start();
  gsmtrx_log( LOG_DEBUG, "Radio started" );
...
void RadioInterface::driveReceiveRadio() {
  ...
  unsigned int rcvSz = rcvCursor/2;
  int readSz = 0;
