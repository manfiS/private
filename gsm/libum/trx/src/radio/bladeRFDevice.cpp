...
{
    gsmtrx_log( LOG_INFO, "Creating bladeRF device..." );
...
bool bladeRFDevice::open(const std::string &args, bool)
{
    ...
    gsmtrx_log( LOG_INFO, "bladeRF library version %d.%d.%d ( %s ).", ver.major, ver.minor, ver.patch, ver.describe );

    int status = bladerf_open(&bdev, args.c_str());
    if (status < 0) {
        gsmtrx_log( LOG_ALERT, "Could not open bladeRF device: %s", bladerf_strerror(status) );
        return false;
    }
    char buf[34];
    ...
    if (status < 0) {
        gsmtrx_log( LOG_WARNING, "Opened bladeRF serial=%s firmware version error: %s", buf, bladerf_strerror(status) );
    }
    else {
        gsmtrx_log( LOG_NOTICE, "Opened bladeRF serial=%s firmware version %d.%d.%d ( %s ).",
          buf,
          ver.major,
          ver.minor,
          ver.patch,
    ...
    if (!(status = bladerf_get_fpga_size(bdev, &bfs))) {
        ...
        if (status < 0) {
            gsmtrx_log( LOG_ALERT, "Error loading FPGA file %s: %s", fpgaName, bladerf_strerror(status) );
            return false;
        }
#endif

    } else {
        gsmtrx_log( LOG_ALERT, "Error fetching FPGA size: %s", bladerf_strerror(status) );
        return false;
    }

    status = bladerf_fpga_version(bdev, &ver);
    if (status < 0) {
        gsmtrx_log( LOG_WARNING, "bladeRF FPGA %s is loaded, version error: %s", fpgaName, bladerf_strerror(status) );
    }
    else {
        gsmtrx_log( LOG_INFO, "bladeRF FPGA %s is loaded with version %d.%d.%d ( %s ).",
          fpgaName,
          ver.major,
          ver.minor,
          ver.patch,
    ...
    bladerf_config_gpio_read(bdev, &val);
    if (!(val & 0x10000)) {
        gsmtrx_log( LOG_ALERT, "Could not enable timestamps" );
        return false;
    }

    gsmtrx_log( LOG_INFO, "bladeRF timestamping enabled" );

    if (BLADERF_DEVICE_SPEED_SUPER != bladerf_device_speed(bdev)) {
        gsmtrx_log( LOG_EMERG, "bladeRF transceiver only supports SuperSpeed mode at the moment." );
        return false;
    }
    ...
    if ((status = bladerf_set_rational_sample_rate(bdev, BLADERF_MODULE_RX, &rate, &actual) < 0)) {
        gsmtrx_log( LOG_ALERT, "Error setting RX sampling rate: %s", bladerf_strerror(status) );
        return false;
    }
    ...
    if ((status = bladerf_set_rational_sample_rate(bdev, BLADERF_MODULE_TX, &rate, &actual)) < 0) {
        gsmtrx_log( LOG_ALERT, "Error setting TX sampling rate: %s", bladerf_strerror(status) );
        return false;
    }

    unsigned int bw = 0;
    if ((status = bladerf_set_bandwidth(bdev, BLADERF_MODULE_RX, BANDWIDTH, &bw) < 0)) {
        gsmtrx_log( LOG_ALERT, "Error setting RX bandwidth: %s", bladerf_strerror(status) );
    }

    if ((status = bladerf_set_bandwidth(bdev, BLADERF_MODULE_TX, BANDWIDTH, &bw) < 0)) {
        gsmtrx_log( LOG_ALERT, "Error setting TX bandwidth: %s", bladerf_strerror(status) );
    }
    else {
        gsmtrx_log( LOG_INFO, "Actual bandwidth %d", bw );
    }
    ...
    if (status < 0)
        gsmtrx_log( LOG_CRIT, "Failed to intialize RX sync handle: %s", bladerf_strerror(status) );
    ...
    if (status < 0)
        gsmtrx_log( LOG_CRIT, "Failed to intialize TX sync handle: %s", bladerf_strerror(status) );
...
{
  gsmtrx_log( LOG_NOTICE, "Starting bladeRF..." );
  ...
  if ((status = bladerf_enable_module(bdev, BLADERF_MODULE_RX, 1)) < 0) {
      gsmtrx_log( LOG_ALERT, "Error enabling RX: %s", bladerf_strerror(status) );
      return false;
  }

  if ((status = bladerf_enable_module(bdev, BLADERF_MODULE_TX, 1)) < 0) {
      gsmtrx_log( LOG_ALERT, "Error enabling TX: %s", bladerf_strerror(status) );
      return false;
  }
...
double bladeRFDevice::setTxGain(double dB)
{
    ...
    writeLock.unlock();
    if (status < 0) {
        gsmtrx_log( LOG_ERR, "Error setting TX gain: %s", bladerf_strerror(status) );
    }
...
double bladeRFDevice::setRxGain(double dB)
{
    ...
    writeLock.unlock();
    if (status < 0) {
        gsmtrx_log( LOG_ERR, "Error setting RX gain: %s", bladerf_strerror(status) );
    }
        ...
        unsigned *RSSI)
{
    ...
    while (len > 0) {
        ...
        if (++rxBufIndex >= rxBufCount) {
            ...
            if (status < 0) {
                gsmtrx_log( LOG_ERR, "Samples RX failed at %llu: %s", rxTimestamp, bladerf_strerror(status) );
                break;
            }
            rxBufCount = bufCount;
            ...
            if (mDcCorrect && ((corrI != mRxCorrectionI) || (corrQ != mRxCorrectionQ))) {
                gsmtrx_log( LOG_INFO, "Adjusting Rx DC offsets: %d %d", corrI, corrQ );
        ...
        if (tStamp != rxTimestamp) {
            ...
            if (abs(delta) < 1000 || (tStamp == rxResyncCandidate)) {
                gsmtrx_log( LOG_NOTICE, "RX Timestamp adjusted by %lld to %llu.", delta, tStamp );
                rxTimestamp = tStamp;
                rxResyncCandidate = 0;
                if (overrun && (delta > 0))
                    *overrun = true;
                ...
                gsmtrx_log( LOG_WARNING, "RX Timestamp jumped by %lld at %llu in buffer %d / %d",
                  delta,
                  rxTimestamp,
                  rxBufIndex,
                  rxBufCount );
...
//    ::write(bf,buf,4*len);
    ...
    if (timestamp != tStamp) {
        gsmtrx_log( LOG_WARNING, "TX Timestamp difference: %llu vs %llu", timestamp, tStamp );
        txBuffered = 0;
    ...
    while (txBuffered >= PKT_SAMPLES_USE) {
        ...
        if (status < 0) {
            gsmtrx_log( LOG_ERR, "Samples TX failed at %llu: %s", txTimestamp, bladerf_strerror(status) );
        }
        else
...
bool bladeRFDevice::setLoopback(bladerf_loopback loopback)
{
    ...
    writeLock.unlock();
    if (status < 0) {
        gsmtrx_log( LOG_ERR, "Setting loopback mode failed: %s", bladerf_strerror(status) );
        return false;
    }
    return true;
}
...
{
    gsmtrx_log( LOG_INFO, "set VCTCXO: %u", wAdjFreq );
...
bool bladeRFDevice::setRxOffsets(int corrI, int corrQ)
{
    ...
    writeLock.unlock();
    if (status < 0) {
        gsmtrx_log( LOG_ERR, "Setting RX DC correction failed: %s", bladerf_strerror(status) );
        return false;
    }
    return true;
}
...
bool bladeRFDevice::setTxOffsets(int corrI, int corrQ)
{
    ...
    writeLock.unlock();
    if (status < 0) {
        gsmtrx_log( LOG_ERR, "Setting TX DC correction failed: %s", bladerf_strerror(status) );
        return false;
    }
    return true;
}
...
bool bladeRFDevice::runCustom(const std::string &command)
{
    if (command == "normal" || command == "on") {
        ...
        if (status >= 0)
            ...
            gsmtrx_log( LOG_ERR, "Error setting RX gain1: %s", bladerf_strerror(status) );
        }
        writeLock.unlock();
    }
