	...
	:mClockSocket(wBasePort,TRXAddress,wBasePort+100)
{
  ...
  GSM::Time startTime = mStartTime = GSM::Time(random() % gHyperframe,0);
	...
	gsmtrx_log( LOG_DEBUG, "Started CLOCK socket on %s:%d", TRXAddress, wBasePort );
  ...
  gsmtrx_log( LOG_NOTICE, "Running %d ARFCNs with oversampling %d", mNumARFCNs, wOversamplingRate );
...
void Transceiver::setFiller(radioVector *rv, bool allocate, bool force)
{
	...
	if (!force && (IGPRS == mChanType[CN][TN])) {
		gsmtrx_log( LOG_INFO, "setFiller ignored: CN=%d TN=%d", CN, TN );
...
void Transceiver::pushRadioVector(GSM::Time &nowTime)
{
  ...
  while (radioVector *next = (radioVector*) mTransmitPriorityQueue.getCurrentBurst(nowTime)) {
	...
	if (CN >= mNumARFCNs) {
	  gsmtrx_log( LOG_ERR, "Attempt to send burst on illegal ARFCN. C%d T%d FN %d", CN, TN, nowTime.FN() );
      delete next;
	  continue;
	}
	if (addFiller[CN] == false) {
	  gsmtrx_log( LOG_ERR, "Attempt to send multiple bursts on C%d T%d FN %d", CN, TN, nowTime.FN() );
      delete next;
	  continue;
	}
...
void Transceiver::driveControl(unsigned ARFCN)
{
  ...
  if (strcmp(command,"POWEROFF")==0) {
    ...
    if ((ARFCN==0) && !mRadioInterface->tuneRx(mRxFreq,128)) {
    ...
    if ((ARFCN==0) && !mRadioInterface->tuneTx(mTxFreq,128)) {
...
bool Transceiver::driveTransmitPriorityQueue(unsigned ARFCN)
{
  ...
  if (msgLen!=gSlotLen+1+4+1) {
    gsmtrx_log( LOG_ERR, "Badly formatted packet on GSM->TRX interface" );
    return false;
  }
  ...
  gsmtrx_log( LOG_DEBUG, "rcvd. burst" );
